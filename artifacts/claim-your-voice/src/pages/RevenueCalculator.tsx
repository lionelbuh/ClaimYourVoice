...
function FounderView({ inputs, outputs, updateInput, resetToDefaults, viewPeriod }: { 
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = "cyv-projections.csv";
    a.click();
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <InputSlider
                ...
                step={1}
                suffix="%"
                tooltip="Percentage of coaching fees kept by the platform (specialists keep the rest)"
              />
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <InputSlider
                label="Guide Payout Rate (Subscriptions)"
                ...
                step={5}
                suffix="%"
                tooltip="Percentage of subscription revenue paid to guides (applies to $49/mo membership only)"
              />
              <div className="bg-slate-50 dark:bg-slate-900 rounded-lg p-3 text-sm">
                <div className="flex items-center gap-2 text-muted-foreground mb-1">
                  <Info className="h-4 w-4" />
                  <span className="font-medium">Guide Payment Estimate (Subscriptions)</span>
                </div>
                <p className="text-slate-600 dark:text-slate-400">
                  At {inputs.mentorPayoutRate}% of ${inputs.subscriptionPrice}, each member generates <span className="font-semibold text-indigo-600">${((inputs.mentorPayoutRate / 100) * inputs.subscriptionPrice).toFixed(2)}/month</span> for their guide.
                </p>
              </div>
              <InputSlider
                label="Guide Payout on Specialists Profit"
                ...
                step={5}
                suffix="%"
                tooltip={`Percentage of the platform's ${inputs.platformCommissionRate}% coaching commission shared with guides`}
              />
              <div className="bg-slate-50 dark:bg-slate-900 rounded-lg p-3 text-sm">
                <div className="flex items-center gap-2 text-muted-foreground mb-1">
                  <Info className="h-4 w-4" />
                  <span className="font-medium">Guide Coaching Bonus</span>
                </div>
                <p className="text-slate-600 dark:text-slate-400">
                  Guides receive <span className="font-semibold text-indigo-600">{inputs.mentorCoachingPayoutRate || 0}%</span> of ClaimYourVoice's {inputs.platformCommissionRate}% coaching commission. If a specialist earns ${inputs.avgCoachingSpendPerUser}, the platform keeps ${(inputs.avgCoachingSpendPerUser * inputs.platformCommissionRate / 100).toFixed(0)} ({inputs.platformCommissionRate}%), and guides receive <span className="font-semibold text-indigo-600">${((inputs.avgCoachingSpendPerUser * inputs.platformCommissionRate / 100) * ((inputs.mentorCoachingPayoutRate || 0) / 100)).toFixed(2)}</span>.
                </p>
              </div>
              <InputSlider
                label="Guide Welcome Call Payment"
                ...
                step={5}
                prefix="$"
                tooltip="One-time payment to guide for the first 30-minute welcome call with each new member"
              />
              <div className="bg-slate-50 dark:bg-slate-900 rounded-lg p-3 text-sm">
                <div className="flex items-center gap-2 text-muted-foreground mb-1">
                  <Info className="h-4 w-4" />
                ...
                <p className="text-slate-600 dark:text-slate-400">
                  Each new member triggers a one-time <span className="font-semibold text-indigo-600">${inputs.mentorWelcomeCallPayment}</span> payment to their assigned guide for the first 30-minute onboarding call.
                </p>
              </div>
              <InputSlider
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Guide Payouts</span>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground mt-2">
                {isPublic 
                  ? "Understand what's possible when you build with ClaimYourVoice"

...
export default function Login() {
  ...
  const handleGoToDashboard = () => {
    ...
    else if (userRole === "member") navigate("/dashboard-member");
    else if (userRole === "guide") navigate("/dashboard-guide");
    else if (userRole === "specialist") navigate("/dashboard-specialist");
    else if (userRole === "supporter") navigate("/dashboard-supporter");
  };
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.session) {
        ...
        const authKeys = Object.keys(localStorage).filter(k => k.includes('sb-') || k.includes('supabase') || k.includes('cyv-'));
      ...
      if (data.user) {
        ...
        if (userEmail) {
          ...
          if (entrepreneurApp && entrepreneurApp.length > 0) {
            ...
            console.log("[LOGIN] Found approved/pre-approved member application, status:", entrepreneurApp[0].status);
          }
          ...
          if (!applicationRole) {
            ...
            if (mentorApp && mentorApp.length > 0) {
              ...
              console.log("[LOGIN] Found approved guide application");
            }
          }
          ...
          if (!applicationRole) {
            ...
            if (coachApp && coachApp.length > 0) {
              applicationRole = "specialist";
              console.log("[LOGIN] Found approved specialist application");
            }
          }
          ...
          if (!applicationRole) {
            ...
            if (investorApp && investorApp.length > 0) {
              applicationRole = "supporter";
              console.log("[LOGIN] Found approved supporter application");
            }
          }
        ...
        if (!userRole) {
          userRole = data.user.user_metadata?.user_type || "member";
        ...
        else if (userRole === "guide") dashboardPath = "/dashboard-guide";
        else if (userRole === "specialist") dashboardPath = "/dashboard-specialist";
        else if (userRole === "supporter") dashboardPath = "/dashboard-supporter";
        
        const currentParams = new URLSearchParams(window.location.search);
        const paymentStatus = currentParams.get("payment");
        const sessionId = currentParams.get("session_id");
        if (paymentStatus && dashboardPath === "/dashboard-member") {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      if (selectedRole === "member") navigate("/member-focus");
      else if (selectedRole === "guide") navigate("/become-guide");
      else if (selectedRole === "specialist") navigate("/become-specialist");
      else if (selectedRole === "supporter") navigate("/supporters");
    } catch (error: any) {
  ...
  const handleRoleSelection = (role: string) => {
    ...
    if (role === "member") navigate("/member-focus");
    else if (role === "guide") navigate("/become-guide");
    else if (role === "specialist") navigate("/become-specialist");
    else if (role === "supporter") navigate("/supporters");
  };
      ...
      <div className="min-h-screen flex" style={{ backgroundColor: "#FAF9F7" }}>
        <div className="hidden md:flex md:w-1/2 items-center justify-center p-12" style={{ backgroundColor: "#0D566C" }}>
          <div className="max-w-sm text-center">
            <h2 className="text-3xl font-display font-bold text-white mb-4">Welcome back to ClaimYourVoice</h2>
      ...
      <div className="hidden md:flex md:w-1/2 items-center justify-center p-12" style={{ backgroundColor: "#0D566C" }}>
        <div className="max-w-sm text-center">
          ...
          <p className="leading-relaxed" style={{ color: "rgba(255,255,255,0.75)" }}>
            AI helps organize your thoughts. Guides help you make them real. Sign in to continue your journey.
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md">
          ...
          {!isRecovery && isLogin && (
            <div className="bg-white rounded-2xl p-8 md:p-10" style={cardStyle}>
              ...
              <p className="mb-8" style={{ color: "#8A8A8A" }}>Sign in to your ClaimYourVoice account</p>
          ...
          {!isRecovery && !isLogin && showRoles && (
            <div className="bg-white rounded-2xl p-8 md:p-10" style={cardStyle}>
              <h2 className="text-2xl md:text-3xl font-display font-bold mb-2" style={{ color: "#0D566C" }}>Join ClaimYourVoice</h2>
              <p className="mb-8" style={{ color: "#8A8A8A" }}>Select your role to get started</p>
              <div className="space-y-3 mb-6">
                {[
                  { role: "member", icon: Lightbulb, color: "#F5C542", label: "I'm an Member", desc: "Submit your idea and get guidance", testId: "button-role-member" },
                  { role: "guide", icon: Star, color: "#4B3F72", label: "I'm a Guide", desc: "Guide and support members", testId: "button-role-guide" },
                  { role: "specialist", icon: Briefcase, color: "#0D566C", label: "I'm a Specialist", desc: "Offer coaching services hourly", testId: "button-role-specialist" },
                  { role: "supporter", icon: TrendingUp, color: "#FF6B5C", label: "I'm an Supporter", desc: "Invest in journeys and members", testId: "button-role-supporter" },
                ].map((item) => {
          ...
          {!isRecovery && !isLogin && !showRoles && (
            <div className="bg-white rounded-2xl p-8 md:p-10" style={cardStyle}>
              ...
              <p className="mb-8" style={{ color: "#8A8A8A" }}>Create your ClaimYourVoice account</p>
              <div className="space-y-5">
                <div className="space-y-1">
                  <p className="font-medium" style={{ color: "#4B3F72" }}>You are an Member with an idea</p>
                </div>

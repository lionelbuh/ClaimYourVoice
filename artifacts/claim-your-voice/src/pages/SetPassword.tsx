  ...
  const handleSetPassword = async () => {
    ...
    if (token) {
      ...
      if (userType === "specialist") {
        const specialists = JSON.parse(localStorage.getItem("tcp_coachApplications") || "[]");
        const specialist = specialists.find((c: any) => c.email === email);
        if (specialist) {
          specialist.password = password;
          specialist.hasPassword = true;
          localStorage.setItem("tcp_coachApplications", JSON.stringify(specialists));
          localStorage.removeItem("tcp_setPasswordPending");
          window.scrollTo(0, 0);
          navigate("/dashboard-specialist");
        }
      } else if (userType === "guide") {
        const guides = JSON.parse(localStorage.getItem("tcp_mentorApplications") || "[]");
        const guide = guides.find((m: any) => m.email === email);
        if (guide) {
          guide.password = password;
          guide.hasPassword = true;
          localStorage.setItem("tcp_mentorApplications", JSON.stringify(guides));
          localStorage.removeItem("tcp_setPasswordPending");
          window.scrollTo(0, 0);
          navigate("/dashboard-guide");
        }
      } else if (userType === "supporter") {
        const supporters = JSON.parse(localStorage.getItem("tcp_investorApplications") || "[]");
        const supporter = supporters.find((i: any) => i.email === email);
        if (supporter) {
          supporter.password = password;
          supporter.hasPassword = true;
          localStorage.setItem("tcp_investorApplications", JSON.stringify(supporters));
          localStorage.removeItem("tcp_setPasswordPending");
          window.scrollTo(0, 0);
          navigate("/dashboard-supporter");
        }
      } else if (userType === "member") {
        const members = JSON.parse(localStorage.getItem("tcp_entrepreneurApplications") || "[]");
        const member = members.find((e: any) => e.email === email);
        if (member) {
          member.password = password;
          member.hasPassword = true;
          localStorage.setItem("tcp_entrepreneurApplications", JSON.stringify(members));
          localStorage.removeItem("tcp_setPasswordPending");
          window.scrollTo(0, 0);

...
type Tab = "focus" | "plan" | "specialists" | "messages" | "profile";
...
export default function TrialDashboard() {
  ...
  const fetchMentorInfo = async (trialId: number) => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/trial/${trialId}/guide-info`);
      const data = await res.json();
      if (data.assigned) {
        setMentorInfo({ mentorName: data.mentorName, mentorEmail: data.mentorEmail });
      }
    } catch (err) {
      console.error("Failed to fetch guide info:", err);
    }
  };
  ...
  const tabs: { id: Tab; label: string; icon: React.ReactNode; locked: boolean }[] = [
    ...
    { id: "plan", label: "Personal Action Plan", icon: <FileText className="h-4 w-4" />, locked: true },
    { id: "specialists", label: "Specialists", icon: <GraduationCap className="h-4 w-4" />, locked: true },
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#FAF9F7" }}>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-display font-bold" style={{ color: "#0D566C" }} data-testid="text-dashboard-title">Member Dashboard</h1>
                ...
                <div className="flex-1 space-y-3 mb-4 max-h-[400px] overflow-y-auto">
                  {loadingMessages ? (
                    ...
                    <div className="text-center py-12" style={{ color: "#8A8A8A" }}>
                      ...
                      <p>No messages yet. Start a conversation with your guide.</p>
        ...
        {(activeTab === "plan" || activeTab === "specialists" || activeTab === "profile" || (activeTab === "messages" && !trialStatus.mentorId)) && (
          <Card className="rounded-2xl" style={{ backgroundColor: "#FFFFFF", borderColor: "#E8E8E8", boxShadow: "0 2px 12px rgba(224,224,224,0.6)" }}>
            <CardContent className="py-16 text-center">
              ...
              <h3 className="text-xl font-bold mb-2" style={{ color: "#0D566C" }} data-testid="text-locked-section">
                {activeTab === "messages" ? "Guide Not Yet Assigned" : "Available After Membership"}
              </h3>
              <p className="max-w-md mx-auto mb-6" style={{ color: "#8A8A8A" }}>
                {activeTab === "messages" 
                  ? "An admin will assign a guide to your trial account. You'll be able to message them directly once assigned."
                  : "This section becomes available when you become a full member. Apply to unlock your complete member dashboard."
                }
              </p>
              {activeTab !== "messages" && (
                <Link href="/become-member">
          ...
          <Card className="rounded-2xl mt-8" style={{ backgroundColor: "#0D566C", borderColor: "rgba(13,86,108,0.3)" }}>
            <CardContent className="p-8 text-center">
              ...
              <p className="mb-6 max-w-lg mx-auto" style={{ color: "rgba(255,255,255,0.75)" }}>
                Your trial has ended but your progress is saved. Apply to become a full member and unlock dedicated mentorship, personal action planning tools, and the complete member dashboard.
              </p>
              <Link href="/become-member">

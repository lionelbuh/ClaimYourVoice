...
export default function InsightArticle() {
  ...
  useEffect(() => {
    if (article) {
      document.title = `${article.title} | ClaimYourVoice Insights`;
      setMetaTag("description", article.summary);
      setMetaTag("og:title", `${article.title} | ClaimYourVoice Insights`);
      setMetaTag("og:description", article.summary);
      setMetaTag("og:type", "article");
      setMetaTag("og:url", `https://www.claimyourvoice.org/insights/${article.slug}`);
    }
  }, [article]);
  ...
  const jsonLd = {
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `https://claimyourvoice.org/insights/${article.slug}`
    }
  };
        ...
        <div className="container px-4 mx-auto max-w-3xl">
          <Link href="/insights" data-testid="link-back-insights">
            <span className="inline-flex items-center gap-1.5 text-sm font-medium mb-6 cursor-pointer transition-colors duration-200" style={{ color: "#FF6B5C" }}>
              <ArrowLeft className="h-4 w-4" /> Back to Journey Radar
            </span>
          </Link>
          <span
          ...
          <article className="space-y-10">
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-lg font-display font-bold text-white">How ClaimYourVoice Helps</h2>
              </div>
              <p className="leading-relaxed mb-6" style={{ color: "rgba(255,255,255,0.85)" }}>{article.howWeHelp}</p>
              <Link href="/member-focus" data-testid="link-article-cta">
                <Button

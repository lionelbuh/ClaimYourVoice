...
export default function CoachProfile() {
  ...
  const [specialist, setCoach] = useState<CoachData | null>(null);
  ...
  useEffect(() => {
    const fetchCoach = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/specialists/${coachId}`);
        if (response.ok) {
    ...
    const fetchRating = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/specialist-ratings/${coachId}`);
        if (response.ok) {
          const data = await response.json();
          setRating(data);
        }
    ...
    const fetchReviews = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/specialist-ratings/${coachId}/reviews`);
        if (response.ok) {
          const data = await response.json();
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: specialist?.full_name ? `${specialist.full_name} - Specialist at ClaimYourVoice` : "Specialist Profile",
          text: specialist?.bio || "Check out this amazing specialist!",
          url: url,
        });
      } catch (error) {
        copyToClipboard(url);
    ...
    return (
      <div className="min-h-screen" style={{ backgroundColor: "#FAF9F7" }}>
        <div className="container mx-auto px-4 py-12 text-center">
          ...
          <h1 className="text-2xl font-bold mb-2" style={{ color: "#0D566C" }}>Specialist Not Found</h1>
          <p className="mb-6" style={{ color: "#8A8A8A" }}>This specialist profile doesn't exist or has been removed.</p>
          <Link href="/">
  ...
  let rates = null;
  try {
    rates = JSON.parse(specialist.hourly_rate);
  } catch {}

  const focusAreas = specialist.focus_areas?.split(",").map(a => a.trim()).filter(a => a) || [];
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#FAF9F7" }}>
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <Card className="overflow-hidden rounded-2xl" style={{ backgroundColor: "#FFFFFF", borderColor: "#E8E8E8", boxShadow: "0 2px 12px rgba(224,224,224,0.6)" }}>
          ...
          <div className="px-6 md:px-8 py-6">
            <div className="text-center mb-6">
              ...
              <p className="text-lg mt-1" style={{ color: "#8A8A8A" }}>{specialist.expertise}</p>
          ...
          <CardContent className="px-6 md:px-8 py-6 space-y-6">
            {specialist.bio && (
              <div>
                ...
                <p className="whitespace-pre-line" style={{ color: "#4A4A4A" }}>{specialist.bio}</p>
              </div>
            )}

            {specialist.external_reputation && (
              <div className="rounded-xl p-4" style={{ backgroundColor: "rgba(245,197,66,0.08)", border: "1px solid rgba(245,197,66,0.25)" }}>
                <div className="flex items-center gap-3 flex-wrap">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="ml-2 font-bold text-lg" style={{ color: "#4A4A4A" }}>{specialist.external_reputation.average_rating}</span>
                    <span style={{ color: "#8A8A8A" }}>({specialist.external_reputation.review_count} reviews)</span>
                  </div>
                </div>
                <p className="text-sm mt-2" style={{ color: "#8A8A8A" }}>
                  Based on self-reported ratings from {specialist.external_reputation.platform_name}
                </p>
                {specialist.external_reputation.verified && (
                  <div className="flex items-center gap-2 mt-2">
                    <Check className="h-4 w-4 text-green-600" />
                    <span className="text-sm font-medium text-green-700">
                      Externally verified by ClaimYourVoice
                    </span>
                  </div>
                )}
              </div>
            ...
            {rates && rates.introCallRate && (
              <div>
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <a href="https://www.claimyourvoice.org/member-focus" className="block">
                ...
                <p className="text-sm text-center mt-4" style={{ color: "#8A8A8A" }}>
                  ...
                  <a href="https://www.claimyourvoice.org/member-focus" className="hover:underline" style={{ color: "#FF6B5C" }}>Take the free SM Journey Score</a> to get started and unlock coaching sessions.
                </p>
              </div>
            )}

            <div className="pt-4" style={{ borderTop: "1px solid #E8E8E8" }}>
              <a href="https://www.claimyourvoice.org/member-focus">
                <Button
                  variant="outline"
                  size="lg"
          ...
          <DialogContent className="sm:max-w-md rounded-2xl" style={{ backgroundColor: "#FFFFFF" }}>
            <DialogHeader>
              ...
              <DialogDescription style={{ color: "#8A8A8A" }}>
                To contact this specialist, you need to register as an member on ClaimYourVoice. It's free to join and takes just a minute.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              ...
              <ul className="list-disc list-inside text-sm space-y-1" style={{ color: "#4A4A4A" }}>
                <li>Connect with expert specialists like {specialist.full_name}</li>
                <li>Get matched with guides for your journey</li>
                <li>Access AI-powered personal action planning tools</li>
        ...
        <div className="mt-8 text-center space-y-6">
          <div>
            <p className="mb-4" style={{ color: "#8A8A8A" }}>Want to grow your journey with a personal guide and expert specialists?</p>
            <Link href="/member-focus">
              <Button className="rounded-full" style={{ backgroundColor: "#FF6B5C", color: "#FFFFFF", border: "none" }} data-testid="button-become-member">
                <UserPlus className="mr-2 h-4 w-4" />
                Join as an Member
              </Button>
            </Link>
          ...
          <div className="pt-6" style={{ borderTop: "1px solid #E8E8E8" }}>
            <p className="mb-4" style={{ color: "#8A8A8A" }}>Want to become a specialist on ClaimYourVoice?</p>
            <Link href="/become-specialist">
              <Button variant="outline" className="rounded-xl" style={{ borderColor: "#0D566C", color: "#0D566C" }} data-testid="button-become-specialist">

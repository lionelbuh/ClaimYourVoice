...
export default function BusinessPlan() {
  ...
  const generateBusinessPlan = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to generate personal action plan");
      }
      ...
      console.error("Personal action plan generation error:", error);
      setGenerationError(error.message || "Failed to generate personal action plan");
      toast.error("Failed to generate personal action plan. Please try again.");
    } finally {
      ...
      <div className="flex min-h-[calc(100vh-4rem)] bg-slate-50 dark:bg-slate-950">
        ...
        <main className="flex-1 p-8 overflow-y-auto flex items-center justify-center">
          <div className="max-w-md text-center">
            ...
            <h1 className="text-3xl font-display font-bold text-slate-900 dark:text-white mb-3">Personal Action Plan Submitted!</h1>
            <p className="text-slate-600 dark:text-slate-400 mb-4">Your personal action plan has been sent to our guide review committee.</p>
            <p className="text-slate-600 dark:text-slate-400 mb-8">You're now on the waiting list. Once our Guides review your idea, the right Guide may choose to take it on — and when that happens, we will contact you to join as a member. Check your dashboard for updates!</p>
            <Button 
              className="w-full bg-cyan-600 hover:bg-cyan-700"
              onClick={() => window.location.href = "/dashboard-member?submitted=true"}
      ...
      <div className="flex min-h-[calc(100vh-4rem)] bg-slate-50 dark:bg-slate-950">
        <aside className="w-64 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 hidden md:flex flex-col">
          <div className="p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <div className="font-bold text-sm">Member</div>
                <div className="text-xs text-muted-foreground">Creating Personal Action Plan</div>
              </div>
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 p-8 overflow-y-auto flex items-center justify-center">
          <div className="text-center max-w-md">
            ...
            <h2 className="text-2xl font-display font-bold text-slate-900 dark:text-white mb-3">Generating Your Personal Action Plan</h2>
            <p className="text-slate-600 dark:text-slate-400">Our AI is analyzing your answers and creating a comprehensive personal action plan tailored to your idea...</p>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-2">
            <h1 className="text-3xl font-display font-bold text-slate-900 dark:text-white">AI Draft Personal Action Plan</h1>
            <Button 
              variant="outline" 
              size="sm"
              onClick={generateBusinessPlan}
          ...
          <p className="text-muted-foreground mb-8">Review and edit the AI-generated personal action plan for <span className="font-semibold text-foreground">{ideaName}</span>. When ready, submit it to our guides for review.</p>
          ...
          <div className="mt-8 p-4 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900/50 rounded-lg flex gap-3 mb-8">
            ...
            <p className="text-sm text-blue-700 dark:text-blue-300">When you submit this personal action plan, you'll be added to our guide waiting list. A guide will be assigned within 5 business days.</p>
          </div>
          ...
          <div className="flex gap-4">
            ...
            >
              {isSubmitting ? "Submitting..." : "Submit for Guide Review"}
            </Button>
          </div>
        </div>
      </main>
